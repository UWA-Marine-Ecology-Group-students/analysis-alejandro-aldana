...
base_model_filtered <- glmmTMB(sample() ~ bait + (1|location), 
...
fit_model_and_extract <- function(pred_vector) {
  ...
  f <- as.formula(paste(
    "sample ~ bait +", pred_str, "+ (1|location)" ##update green parts from your base model
  ))
...
base_stats <- tibble(
  model = "sample ~ bait +", pred_str, "(1|location)",
  predictors = "none",
