...
##----------------------------------------------------------------------------
## 4a. Full sub-sets for the filtered <1000
##----------------------------------------------------------------------------
...
# Directory
outdir <- "./output/models and plots/total abundance"
...
fit_model_and_extract <- function(pred_vector) {
  ...
  if (is.null(m)) {
    return(tibble(
      model = paste("bait +", pred_str, "+ (1|location)"), ##update
      predictors = pred_str,
      AICc = NA,
      logLik = NA,
      mR2 = NA,
...
# Export CSV
write_csv(final_table, file.path(outdir, "TA<1000_best_models.csv"))
...
# Save to CSV
write_csv(failed_models, file.path(outdir, "TA<1000_failed_models.csv"))

##-------------------------- END -----------------------------------------------

##----------------------------------------------------------------------------
## 4b. check your output
##----------------------------------------------------------------------------
...
##----------------------------------------------------------------------------
## 4c. Full sub-sets for the filtered <500
##----------------------------------------------------------------------------
