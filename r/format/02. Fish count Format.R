...
points_outside_periods <- points %>%
  ...
  dplyr::select(opcode, dplyr::any_of(c("opcode", "period")), family, genus, species, number, frame)
...
periods_wrong <- periods %>%
  dplyr::select(opcode, dplyr::any_of(c("opcode", "period")), time_start, time_end, has_end) %>%
  dplyr::distinct() %>%
...
# 6 periods not 60 minutes long (between 56-59 minutes)
